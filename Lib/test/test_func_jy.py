...
class FunctionTypeTestCase(unittest.TestCase):
    ...
    def test_functiontype(self):
        new_abc = types.FunctionType(abc.__code__, {'xyz': 456},
                                     abc.__name__, abc.__defaults__,
                                     abc.__closure__)
        self.assertEqual(new_abc(), 456)

    def test_functiontype_from_globals(self):
        sm = type(globals())()
        sm.update({'xyz': 789})
        sm_abc = types.FunctionType(abc.__code__, sm, abc.__name__,
                                    abc.__defaults__, abc.__closure__)
        self.assertEqual(sm_abc(), 789)
...
class MethodHashCodeTestCase(unittest.TestCase):
    ...
    def test_builtin_method_hashcode(self):
        foo = 'foo'
        self.assertTrue(foo.title is not foo.title)
    ...
    def test_method_hashcode(self):
        class Foo(object):
            def bar(self):
                pass
        foo = Foo()
        self.assertTrue(foo.bar is not foo.bar)
...
class SingleMethodInterfaceTestCase(unittest.TestCase):
    ...
    def test_java_lang_object_methods(self):
        ...
        self.assertTrue(s.startswith('<function return42'))
