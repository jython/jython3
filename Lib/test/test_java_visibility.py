...
class VisibilityTest(unittest.TestCase):
    def test_invisible(self):
        for item in dir(Invisible):
            self.assertTrue(not item.startswith("package"))
            self.assertTrue(not item.startswith("private"))
            self.assertTrue(not item.startswith("protected"))
    ...
    def test_protected_from_python_subclass(self):
        ...
        for cls in PySubVisible, SubPySubVisible:
            s = cls()
            self.assertEqual(Results.PROTECTED_METHOD, s.protectedMethod(0))
            self.assertEqual(Results.OVERLOADED_PROTECTED_METHOD, s.protectedMethod('foo'))
            self.assertEqual(Results.UNUSED, PySubVisible(Results.UNUSED).visibleField)
        ...
        class SubSubclassable(OnlySubclassable):
            pass
        sub = SubSubclassable()
        self.assertTrue(not sub.filledInByConstructor == 0,
        ...
        class TestObserver(Observer):
            def update(observerself, observable, arg):
                self.assertEqual(to, observable)
                self.assertEqual(None, arg)
                self.updated = True
        to.addObserver(TestObserver())
        to.set('k', 'v')
        self.assertTrue(self.updated, "Calling set should notify the added observer")

    def test_visible(self):
        v = Visible()
        self.assertEqual(Results.PUBLIC_FIELD, v.visibleField)
        self.assertEqual(Results.PUBLIC_STATIC_FIELD, Visible.visibleStaticField)
        Visible.visibleStaticField = Results.PUBLIC_STATIC_FIELD + 1
        self.assertEqual(Results.PUBLIC_STATIC_FIELD + 1, Visible.visibleStaticField)
        self.assertEqual(Results.PUBLIC_STATIC_FIELD + 1, Visible.getVisibleStaticField())
        Visible.setVisibleStaticField(Results.PUBLIC_STATIC_FIELD)
        self.assertEqual(Results.PUBLIC_STATIC_FIELD, Visible.visibleStaticField)
        self.assertEqual(Results.PUBLIC_METHOD, v.visibleInstance(0))
        self.assertEqual(Results.OVERLOADED_PUBLIC_METHOD, v.visibleInstance('a'))
        self.assertEqual(Results.EXTRA_ARG_PUBLIC_METHOD, v.visibleInstance(0, 'b'))
        self.assertEqual(Results.OVERLOADED_EXTRA_ARG_PUBLIC_METHOD,
                v.visibleInstance('a', 'b'))
        self.assertEqual(Results.PUBLIC_STATIC_METHOD, Visible.visibleStatic(0))
        self.assertEqual(Results.OVERLOADED_PUBLIC_STATIC_METHOD,
                v.visibleStatic('a'))
        self.assertEqual(Results.EXTRA_ARG_PUBLIC_STATIC_METHOD,
                v.visibleStatic(0, 'a'))
        self.assertEqual(Results.PUBLIC_STATIC_FIELD, Visible.StaticInner.visibleStaticField)
    ...
    def test_java_subclass(self):
        s = SubVisible()
        self.assertEqual(Results.PUBLIC_FIELD, s.visibleField)
        self.assertEqual(Results.PUBLIC_STATIC_FIELD, SubVisible.visibleStaticField)
        self.assertEqual(Results.SUBCLASS_STATIC_OVERRIDE, SubVisible.visibleStatic(3))
        self.assertEqual(Results.SUBCLASS_STATIC_OVERLOAD, SubVisible.visibleStatic(3.0, 'a'))
        self.assertEqual(Results.SUBCLASS_OVERRIDE, s.visibleInstance(3))
        self.assertEqual(Results.SUBCLASS_OVERLOAD, s.visibleInstance(3.0, 'a'))
        self.assertEqual(Results.PACKAGE_METHOD, s.packageMethod())
        # Java methods don't allow direct calling of the superclass method, so it should
        # return the subclass value here.
        self.assertEqual(Results.SUBCLASS_OVERRIDE, Visible.visibleInstance(s, 3))
        self.assertEqual(Results.PUBLIC_STATIC_FIELD, SubVisible.StaticInner.visibleStaticField)

        self.assertEqual(Results.VISIBLE_SHARED_NAME_FIELD, Visible.sharedNameField)
        self.assertEqual(Results.SUBVISIBLE_SHARED_NAME_FIELD, SubVisible.sharedNameField)
        self.assertEqual(Results.VISIBLE_SHARED_NAME_FIELD * 10, Visible().sharedNameField)
        self.assertEqual(Results.SUBVISIBLE_SHARED_NAME_FIELD * 10, s.sharedNameField)


    def test_in_dict(self):
        for c in Visible, SubVisible, VisibleOverride:
            self.assertTrue('visibleInstance' in c.__dict__,
    ...
    def test_interface_combination(self):
        ...
        self.assertEqual(InterfaceCombination.NO_ARG_RESULT, i.getValue(),
                "methods from IFace should be visible on Implementation")
        self.assertEqual(InterfaceCombination.ONE_ARG_RESULT, i.getValue("one arg"),
                "methods from IIFace should be visible on Implementation")
        self.assertEqual(InterfaceCombination.TWO_ARG_RESULT, i.getValue("one arg", "two arg"),
    ...
    def test_super_methods_visible(self):
        ...
        self.assertEqual("hi", HiddenSuper().hi())
    ...
    def test_inner_class_identity(self):
        ...
        self.assertEqual(id(Matryoshka.Outermost), id(Matryoshka.makeOutermost().__class__))
    ...
    def test_super_methods_merged(self):
        ...
        self.assertEqual("a string", synchList.remove(0))
    ...
    def test_interface_methods_merged(self):
        ...
        for impl in Implementation, ExtraString, ExtraClass, ExtraStringAndClass, ExtraClassAndString:
            instance = impl()
            self.assertEqual("String", instance.call("string argument"))
            self.assertEqual("int", instance.call(7))
            self.assertEqual("Class", instance.call(LinkedList))
    ...
    def test_extending_multiple_hidden_classes(self):
        ...
        self.assertEqual("wrapper close", conn.close())
        self.assertEqual("special close", conn.close(7))
...
class JavaClassTest(unittest.TestCase):
    def test_class_methods_visible(self):
        ...
        self.assertEqual(3, len(HashMap.getInterfaces()))

    def test_python_fields(self):
        self.assertEqual('java.util', HashMap.__module__)
        self.assertEqual(Class, HashMap.__class__)
        self.assertEqual(None, HashMap.__doc__)
        self.assertEqual(list(HashMap.__mro__), HashMap.mro())

    def test_python_methods(self):
        s = SomePyMethods()
        self.assertEqual(6, s[3])
        self.assertEqual(2, s.a, "Undefined attributes should go through to __getattr__")
        self.assertEqual(3, s.b, "Defined fields should take precedence")

class CoercionTest(unittest.TestCase):
    def test_int_coercion(self):
        c = Coercions()
        self.assertEqual("5", c.takeInt(5))
        self.assertEqual("15", c.takeInteger(15))
        self.assertEqual("150", c.takeNumber(150))
        self.assertEqual("take with byte arg: 10", Coercions.take(Byte(10)))

    def test_array_coercion(self):
        self.assertEqual("double", Coercions.takeArray(array.zeros('d', 2)))
        self.assertEqual("float", Coercions.takeArray(array.zeros('f', 2)))
        self.assertEqual("4", Coercions.takePyObj(1, 2, 3, 4))
        c = Coercions()
        self.assertEqual("5", c.takePyObjInst(1, 2, 3, 4, 5))
        self.assertEqual("OtherSubVisible[]", c.takeArray([OtherSubVisible()]))
        self.assertEqual("SubVisible[]", c.takeArray([SubVisible()]))
    ...
    def test_iterable_coercion(self):
        def simple_gen():
            ...
            yield 2
            yield 3
        self.assertEqual(6, Coercions.takeIterable(simple_gen()))
    ...
    def test_class_coercion(self):
        ...
        for obj, cls in ((ht, "java.util.Hashtable"), (hm, "java.util.HashMap"), ("abc", "java.lang.String"),
            ...
            self.assertEqual(c.tellClassNameSerializable(obj), "class " + cls)
        self.assertEqual(c.tellClassNameObject(ht), "class java.util.Hashtable")
