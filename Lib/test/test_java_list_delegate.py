...
class CollectionProxyTest(unittest.TestCase):
    ...
    def check_list(self, control, results, list_type_names, initial, test_name):
        for result, type_name in zip(results, list_type_names):
            try:
                ...
                print(result)
            self.assertEqual(len(control), len(result), "%s: length for %s does not match that of list" % (test_name, type_name))
            for control_value, result_value in zip(control, result):
                self.assertEqual(control_value, result_value, "%s: values from %s do not match those from list" % (test_name, type_name))
    ...
    def _list_op_test(self, initial_value, op_func, test_name):
        ...
        if not isinstance(results[0], list):
            for r, n in zip(results[1:], list_type_names[1:]):
                self.assertEqual(results[0], r, '%s: result for list does not match result for java type %s' % (test_name, n) )
        else:
            self.check_list(results[0], results[1:], list_type_names[1:], initial_value, test_name)

    def test_get_integer(self):
        initial_value = list(range(0, 5))
    ...
    def test_get_slice(self):
        ...
        for i in [None] + list(range(-7, 7)):
            for j in [None] + list(range(-7, 7)):
                for k in [None] + list(range(-7, 7)):
        ...
        for i in [None] + list(range(-7, 7)):
            for j in [None] + list(range(-7, 7)):
                for k in [None] + list(range(-7, 7)):
                    ...
                    self._list_op_test(initial_value, make_op_func(i, j, k, list(range(0, 2))), 'set_slice [%s:%s:%s]=range(0,2)' % (i, j, k))
                    self._list_op_test(initial_value, make_op_func(i, j, k, list(range(0, 4))), 'set_slice [%s:%s:%s]=range(0,4)' % (i, j, k))
                    self._list_op_test(initial_value, make_op_func(i, j, k, range(0, 2)), 'set_slice [%s:%s:%s]=xrange(0,2)' % (i, j, k))
                    self._list_op_test(initial_value, make_op_func(i, j, k, self._arraylist_of(list(range(0, 2)))), 'set_slice [%s:%s:%s]=ArrayList(range(0,2))' % (i, j, k))
    ...
    def test_len(self):
        jlist = ArrayList()
        jlist.addAll(list(range(0, 10)))

        self.assertTrue(len(jlist) == 10)
    ...
    def test_iter(self):
        ...
        self.assertTrue(x == list(range(0, 10)))
    ...
    def test_override_len(self):
        ...
        m = MyList()
        m.addAll(list(range(0, 10)))

        self.assertTrue(len(m) == 11)
    ...
    def test_override_iter(self):
        ...
        self.assertTrue(x == list(range(0, 9)))
    ...
    def test_override_getsetdelitem(self):
        ...
        self.assertTrue(m.get(1) == 1)

        m[0] = 3
        self.assertTrue(m.get(0) == 6)
        self.assertTrue(m[0] == 12)

        del m[0]
        self.assertTrue(m.size() == 11)
        self.assertTrue(m.get(10) == 84)
        ...
        for i in [None] + list(range(-7, 7)):
            for j in [None] + list(range(-7, 7)):
                for k in [None] + list(range(-7, 7)):
                    ...
                    sourcePy = list(range(0, 2))
                    sourceJUL = self._arraylist_of(list(range(0, 2)))
                    sourceIter = range(0, 2)
                    ...
                    self.assertEqual(resultPy, resultJUL)
                    self.assertEqual(resultPy, resultIter)

                    self.assertEqual(destPy, destJUL)
                    self.assertEqual(destPy, destIter)
