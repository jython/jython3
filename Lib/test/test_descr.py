...
def dicts():
    ...
    d = {1:2,3:4}
    l1 = []
    for i in list(d.keys()): l1.append(i)
    l = []
...
def dict_constructor():
    ...
    d = dict({1: 2, 'a': 'b'})
    vereq(d, {1: 2, 'a': 'b'})
    vereq(d, dict(list(d.items())))
    vereq(d, dict(iter(d.items())))
    d = dict({'one':1, 'two':2})
    ...
    d = dict(list(zip(list(range(4)), list(range(1, 5)))))
...
for name, expr in list(binops.items()):
    if expr.islower():
        expr = expr + "(a, b)"
    else:
        expr = 'a %s b' % expr
...
for name, expr in list(unops.items()):
    if expr.islower():
        expr = expr + "(a)"
    else:
        expr = '%s a' % expr
...
def spamdicts():
    ...
    def spamdict(d, memo=None):
        import xxsubtype as spam
        sd = spam.spamdict()
        for k, v in list(d.items()): sd[k] = v
        return sd
    # This is an ugly hack:
    ...
    class C(dict):
        ...
        def __init__(self, *a, **kw):
            if a:
                ...
                self.state = a[0]
            if kw:
                for k, v in list(kw.items()): self[v] = k
        def __getitem__(self, key):
...
def inherits():
    ...
    for i in range(256):
        s = madstring("".join(map(chr, list(range(i)))))
        t = s.rev()
        u = t.rev()
        vereq(u, s)
    s = madstring("12345")
...
def keywords():
    ...
    vereq(tuple(sequence=list(range(3))), (0, 1, 2))
...
def dictproxyiterkeys():
    ...
    keys = sorted([ key for key in C.__dict__.keys() ])
...
def dictproxyiteritems():
    ...
    keys = sorted([ key for (key, value) in C.__dict__.items() ])
    vereq(keys, ['__dict__', '__doc__', '__module__', '__weakref__', 'meth'])
