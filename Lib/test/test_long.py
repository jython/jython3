...
special = list(map(int, [0, 1, 2, BASE, BASE >> 1]))
...
special = special + [~x for x in special] + \
                    [-x for x in special]
...
if support.have_unicode:
    L += [
        ...
        (str('314'), 314),
        (str(' 314'), 314),
        (str('\\u0663\\u0661\\u0664 ', 'raw-unicode-escape'), 314),
        (str('  \t\t  314  \t\t  '), 314),
        (str('  1x'), ValueError),
        (str('  1  '), 1),
        (str('  1\02  '), ValueError),
...
class LongTest(unittest.TestCase):
    ...
    def slow_format(self, x, base):
        ...
        return '-'[:sign] + \
               {8: '0', 10: '', 16: '0x'}[base] + \
               "".join(["0123456789abcdef"[i] for i in digits]) + "L"
    ...
    def test_long(self):
        ...
        self.assertEqual(type(int(314)), int)
        self.assertEqual(type(int(3.14)), int)
        self.assertEqual(type(int(314)), int)
...
# ----------------------------------- tests of auto int->long conversion
    ...
    def test_float_conversion(self):
        ...
        for x in exact_values:
            self.assertEqual(int(float(x)), x)
            self.assertEqual(int(float(-x)), -x)
